...
{
	do_check_image ./u-boot.bin
	do_check_image ./boot.img
	do_check_image ./recovery.img
	do_check_image ./system.img
}
...
if [ "${flash_images}" -eq "1" ]; then
    ...
    dd if=./u-boot.bin of=${node} bs=1024 skip=1 seek=1 conv=fsync
    dd if=/dev/zero of=${node} bs=512 seek=1536 count=16 conv=fsync
    dd if=./boot.img of=${part}1 bs=8192 conv=fsync
    dd if=./recovery.img of=${part}2 bs=8192 conv=fsync
    dd if=./system.img of=${part}5 bs=8192 conv=fsync
fi
}
